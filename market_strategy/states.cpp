...
void MarketState::exit_by_stop_loss(const CandleType &candle, Action action) {
  ...
  switch (action) {
    case Action::NONE: _strategy.to_start();
      break;
    case Action::SHORT: _strategy.to_short(candle.close);
      break;
    case Action::LONG: _strategy.to_long(candle.close);
      break;
  }
}
...
void ShortState::process(const CandleType &candle, Action action) {
  ...
  switch (action) {
    case Action::NONE:
    case Action::SHORT: stop_loss_threshold = _strategy.move_stop_loss_short(candle, stop_loss_threshold);
      break;
    case Action::LONG: _strategy.to_long(candle.close);
      break;
  }
...
void LongState::process(const CandleType &candle, Action action) {
  ...
  switch (action) {
    case Action::NONE:
    case Action::LONG: stop_loss_threshold = _strategy.move_stop_loss_long(candle, stop_loss_threshold);
      break;
    case Action::SHORT: _strategy.to_short(candle.close);
      break;
  }
